...
const emptyDB=()=>({schema:4,settings:{startDate:START,startWeight:76.6,goalWeight:73,pillTime:"22:00"},weights:[],foods:[],workouts:[],body:[],pills:[],deleted:[]});
const $=s=>document.querySelector(s), esc=s=>String(s??"").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[m]));
function localDate(d=new Date()){return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`}
function safeParse(x){try{return JSON.parse(x)}catch{return null}}
function norm(src){let d=emptyDB();if(!src||typeof src!=="object")return d;["weights","foods","workouts","body","pills","deleted"].forEach(k=>{if(Array.isArray(src[k]))d[k]=src[k]});if(src.settings)d.settings={...d.settings,...src.settings};return d}
...
function save(){db.schema=4;localStorage.setItem(STORE,JSON.stringify(db));render()}
...
function recordFingerprint(x,type){
 return `${type}|${x.date||""}|${x.time||""}|${x.meal||""}|${x.items||x.value||x.type||x.text||""}|${x.note||""}`;
}
function deleteRecord(type,key){
 ...
 if(!confirm(`确定删除这条${label}吗？\n\n删除后不会再从旧记录中自动恢复。`))return;
 const fp=recordFingerprint(target,type); db.deleted=db.deleted||[]; if(!db.deleted.includes(fp))db.deleted.push(fp);
...
function render(){
 ...
 let dead=new Set(db.deleted||[]); let fs=db.foods.filter(x=>x.date===localDate()&&!dead.has(recordFingerprint(x,"food"))),ws=db.workouts.filter(x=>x.date===localDate()&&!dead.has(recordFingerprint(x,"workout"))),ww=[...db.weights].reverse().find(x=>x.date===localDate()&&!dead.has(recordFingerprint(x,"weight")));
